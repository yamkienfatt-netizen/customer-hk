...
import { DevelopmentFormProps } from '@/props/Form/DevelopmentFormProps';
...
const Default = (developmentFormProps: DevelopmentFormProps) => {
  let captcha;
  ...
  const isFormSelected = inquiryType === developmentFormProps.rendering.dataSource;
  const hideForm = !isPageEditing && !isFormSelected;

  const { t, locale } = useI18n();
  const formConfigurationServerProps = developmentFormProps.rendering.uid
    ? useComponentProps<FormConfigurationServerProps>(developmentFormProps.rendering.uid)
    : undefined;
  ...
  const developmentFormSchema = z.object({
    ...
    companyName: z.string().min(1, {
      message: t(DICTIONARY_CONSTANT.FORMS.ERRORS.MISSING_COMPANY_NAME),
    }),
  ...
  const developmentForm = useForm<z.infer<typeof developmentFormSchema>>({
    ...
    defaultValues: {
      ...
      message: '',
      language: locale(),
      emailConfigurationId: developmentFormProps.rendering.dataSource,
    },
  });

  const onSubmit = (values: z.infer<typeof developmentFormSchema>) => {
    try {
      setIsLoading(true);
      ...
      if (aliyunCaptchaLoaded) {
        ...
        const initCaptcha = () => {
          window.initAliyunCaptcha({
            ...
            element: '#developmentform-captcha-element', // 页面上预留的渲染验证码的元素，与原代码中预留的页面元素保持一致。
            button: '#developmentform-captcha-button', // 触发验证码弹窗的元素。button表示单击登录按钮后，触发captchaVerifyCallback函数。您可以根据实际使用的元素修改element的值
  ...
  return (
    <>
      {(isPageEditing || !formStateType) &&
        (developmentFormProps.fields.Title.value ||
          developmentFormProps.fields.Description.value ||
          developmentFormProps.fields.Content.value) &&
        isFormSelected && (
          <div className="mt-[50px] max-w-[600px] space-y-8 lg:mx-0">
            <Typography variant="p">
              <ScText field={developmentFormProps.fields.Title} />
            </Typography>
            <Typography variant="p">
              <ScText field={developmentFormProps.fields.Description} />
            </Typography>
            <RichTextTypography>
              <ScRichText field={developmentFormProps.fields.Content} />
            </RichTextTypography>
          </div>
        )}
      ...
      {(isPageEditing || formStateType == 'success') && isFormSelected && (
        <div className="mt-[50px] max-w-[600px] lg:mx-0">
          <RichTextTypography>
            <ScRichText field={developmentFormProps.fields.SuccessMessage} />
          </RichTextTypography>
        </div>
      )}
      ...
      {(isPageEditing || formStateType == 'error') && isFormSelected && (
        <div className="mt-[50px] max-w-[600px] lg:mx-0">
          <RichTextTypography>
            <ScRichText field={developmentFormProps.fields.ErrorMessage} />
          </RichTextTypography>
        </div>
      )}
      ...
      {!formStateType && (
        <Form {...developmentForm}>
          <form
            onSubmit={developmentForm.handleSubmit(onSubmit)}
            className={
          ...
          >
            <>
              ...
              <InputField
                control={developmentForm.control}
                name="companyName"
                placeholder={t(DICTIONARY_CONSTANT.FORMS.FIELD_PLACEHOLDERS.COMPANY_NAME)}
                label={t(DICTIONARY_CONSTANT.FORMS.FIELD_LABELS.COMPANY_NAME)}
                required={true}
              />
              ...
              {showCaptcha && (
                ...
                >
                  ...
                  <div id="developmentform-captcha-button" className="invisible">
                    点击弹出验证码B
                  </div>
...
export default withDatasourceCheck()<DevelopmentFormProps>(Default);

export const getServerSideProps: GetServerSideComponentProps = async (rendering, layoutData) => {
  const developmentFormProps = rendering as DevelopmentFormProps;
