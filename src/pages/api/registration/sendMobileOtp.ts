...
const sendMobileOTPApi = async (
  req: SendOtpRequest,
  res: NextApiResponse
): Promise<NextApiResponse | void> => {
  const requestId = randomUUID();
  loggers.sso().info(`sendMobileOtp|${requestId}|START`, {
    requestId,
    requset_url: req.url,
  ...
  if (req.method !== 'POST') {
    loggers.sso().warn(`sendMobileOtp|${requestId}|405|Method Not Allowed`, {
      requestId,
      response_code: 405,
    });
  ...
  try {
    ...
    loggers.sso().info(`sendMobileOtp|${requestId}|Captcha Verification Result`, {
      requestId,
    ...
    if (!otpToken) {
      loggers.sso().warn(`sendMobileOtp|${requestId}|400|Missing Token`, {
        requestId,
        response_code: 400,
    ...
    if (verifyResp !== 'ok') {
      if (verifyResp == 'exceed') {
        loggers.sso().warn(`sendMobileOtp|${requestId}|400|OtpRateLimitExceed`, { requestId });
        return res.status(400).send({ ErrorCode: 'E306' });
      }
      loggers.sso().info(`sendMobileOtp|${requestId}|400|${verifyResp}-token`, {
        requestId,
        response_code: 400,
    ...
    if (0 && req.body?.Channel == 'email') {
      ...
      markOtpTokenUsed(otpToken);
      loggers.sso().info(`sendMobileOtp|${requestId}|200|Response`, {
        requestId,
        api_request_time_msec: totalTimeInMs,
        response_code: 200,
        response_data: resp,
      });
      return res.status(200).send(Object.assign({}, resp, { Token: generateOtpToken(otpToken) }));
    } else if (req.body?.Channel == 'phone' && !(process.env.SSO_REG_DISABLE_MOBILE === 'true')) {
    ...
    if (e?.response?.data) {
      if (
        ...
        markOtpTokenUsed(otpToken);
        loggers.sso().error(`sendMobileOtp|${requestId}|200|Response`, {
          requestId,
          api_request_time_msec: totalTimeInMs,
      ...
      loggers.sso().error(`sendMobileOtp|${requestId}|${e.response.status}|Response`, {
        requestId,
        api_request_time_msec: totalTimeInMs,
        api_response_data: e.response.data,
      });
  ...
  loggers.sso().warn(`sendMobileOtp|${requestId}|400|Unhandled Request`, { requestId });
  return res.status(400).send({});
};

export default sendMobileOTPApi;
